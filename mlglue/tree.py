...
class Tree:
    ...
    def __init__(self, node_id, children, parent, depth, payload):
        self.id = node_id
    ...
    def to_tmva(self, nodetree, scale, index_mapping):
        ...
        if len(self.children) == 0:
            return (
                '<Node pos="{0}" depth="{1}" NCoef="0" '.format(
                    kind, self.depth + 1)
                + 'IVar="{0}" Cut="{1:.17E}" cType="1" '.format(-1, 0.0)
                + 'res="{0:.17E}" rms="0.0e-00" '.format(
        ...
        # handle non-leaf node
            ...
            return (
                '<Node pos="{0}" depth="{1}" NCoef="0" '.format(
                    kind, self.depth + 1)
                + 'IVar="{0}" Cut="{1:.17E}" cType="1" '.format(
                    index_mapping[self.payload[1]], self.payload[2])
...
def sklearn_to_nodetree(
    ...
    """Recursively converts a sklearn GradientBoosting{Classifier,Regressor}
...
class BDT(object):
    ...
    def to_tmva(self, outfile_name, mva_name="bdt"):
        """ Create list of variables. We assume that all variables are
        'simple', that is, not expressions"""
        varstring = ""
        index_mapping = {
            feature_name: i for i, feature_name in enumerate(self.feature_names)
        }
        for i in range(len(self.feature_names)):
            varstring += (
                '<Variable VarIndex="{0}" Expression="{1}" '.format(
                    i, self.feature_names[i])
        ...
        if self.kind == "regression":
            ...
            for itgt, tgtname in enumerate(self.target_names):
                target_string += (
                    '<Target Name="{0}" TargetIndex="{1}" '.format(
                        tgtname, itgt)
                    + 'Expression="{0}" Label="{0}" Title="{0}" Unit="" '.format(tgtname)
                    + 'Internal="{0}" Type="F" Min="{1:.64E}" '.format(
                        tgtname, 0.0)
                    + 'Max="{0:.64E}"/>\n'.format(0.0)
                )
        ...
        # Loop over decision trees, in scikit that's a 2D array
        # (N_estimators, N_classes) if binary classification, N_classes = 1
        itree = 0
